...
// in-range values.
// The int32 bounds are EXCLUSIVE: MaxQuota itself is not a representable
// quota (fork invariant enforced by the top-up validation tests).
...
// saturateQuotaBounded is the generic variant used by the 64-bit wallet
// domain (WalletQuotaFromDecimalStrict), where the bound itself remains
// representable and only values beyond it are rejected.

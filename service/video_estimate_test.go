...
// TestGeminiRequestIsPricedByTheEndpoint pins the translation the estimate call
// makes for a Gemini-shaped request. The endpoint speaks the OpenAI shape, so a
// Gemini request cannot be passed through as-is; skipping it left an inline
// video with no source that could price it once the local parser (mp4/mov only)
// gave up. Text must survive the translation, or the endpoint would price the
// media alone and under-count the prompt.
func TestGeminiRequestIsPricedByTheEndpoint(t *testing.T) {
	body := []byte(`{"contents":[{"role":"user","parts":[` +
		`{"text":"what happens in this clip?"},` +
		`{"inlineData":{"mimeType":"video/webm","data":"AAAA"}}]}]}`)
	var request dto.GeminiChatRequest
	if err := json.Unmarshal(body, &request); err != nil {
		t.Fatalf("unmarshal fixture: %v", err)
	}

	messages, key, ok := estimateMessagesFor(&request)
	if !ok {
		t.Fatal("a Gemini request with an inline video must be estimable")
	}
	if key == "" {
		t.Fatal("the video must key the cache")
	}
	rendered, ok := messages.([]dto.Message)
	if !ok || len(rendered) != 1 {
		t.Fatalf("expected one message, got %#v", messages)
	}
	parts := rendered[0].ParseContent()
	...
	for _, part := range parts {
		...
		case dto.ContentTypeVideoUrl:
			media := part.GetVideoUrl()
			sawVideo = media != nil && strings.Contains(media.Url, "data:video/webm;base64,AAAA")
		}
	}
	if !sawText {
		t.Fatal("the text part must survive the translation")
	}
	if !sawVideo {
		t.Fatal("the inline video must reach the endpoint as a video_url part")
	}

	// A Gemini request without media is not estimate work.
	var textOnly dto.GeminiChatRequest
	if err := json.Unmarshal([]byte(`{"contents":[{"role":"user","parts":[{"text":"1+1=?"}]}]}`), &textOnly); err != nil {
		t.Fatalf("unmarshal fixture: %v", err)
	}
	if _, _, ok := estimateMessagesFor(&textOnly); ok {
		t.Fatal("a Gemini text request must not be sent to the endpoint")
	}
}

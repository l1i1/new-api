...
func saturatingTokenTotal(promptTokens, completionTokens int) int {
	if promptTokens < 0 {
		promptTokens = 0
	}
...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	...
	summary.TotalTokens = saturatingTokenTotal(usage.PromptTokens, usage.CompletionTokens)
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if mark := GetOpsCyberPolicy(ctx); mark != nil {
		other.SetPublic("cyber_policy", true)
		other.SetPublic("request_type", "cyber")
		other.SetPublic("upstream_error_code", mark.Code)
		other.SetPublic("input_tokens", mark.UpstreamInTok)
		other.SetPublic("output_tokens", mark.UpstreamOutTok)
	}
	...
	relayInfo.ObservedInputTokens = int64(summary.PromptTokens)
	relayInfo.ObservedCacheReadTokens = int64(summary.CacheTokens)
	relayInfo.ObservedCacheWriteTokens = int64(summary.CacheCreationTokens)
	gopool.Go(func() {

...
func SetApiRouter(router *gin.Engine) {
	...
	partnerInternal.Use(middleware.PartnerLimiter(middleware.PaymentWebhookRateLimit("partner_console")))
	{
		...
		partnerInternal.POST("/invite-code/validate", controller.PartnerInviteCodeValidate)
		partnerInternal.PUT("/config", controller.PartnerConfig)
		partnerInternal.GET("/content", controller.PartnerContent)
	}
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		apiRouter.GET("/tokeness/business-analysis", middleware.AdminAuth(), controller.GetBusinessAnalysis)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.GET("/invite-topup-rewards", middleware.DisableCache(), controller.GetInviteTopUpRewards)
				...
				selfRoute.POST("/hotpay/pay", middleware.CriticalRateLimit(), controller.RequestHotPayPay)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/:id/model-rate-limits", controller.GetUserModelRateLimits)
				adminRoute.PUT("/:id/model-rate-limits", controller.ReplaceUserModelRateLimits)
				adminRoute.GET("/:id", controller.GetUser)
				adminRoute.POST("/", controller.CreateUser)
				adminRoute.POST("/manage", controller.ManageUser)
				adminRoute.PUT("/", controller.UpdateUser)
				adminRoute.PUT("/official-fit", controller.UpdateUserOfficialFit)
				adminRoute.GET("/official-fit/families", controller.GetOfficialFitFamilies)
		...
		subscriptionRoute.Use(middleware.UserAuth())
		{
			...
			subscriptionRoute.POST("/hotpay/pay", middleware.CriticalRateLimit(), controller.SubscriptionRequestHotPayPay)
		}
		...
		groupModelRateLimitRoute := apiRouter.Group("/group-model-rate-limits")
		groupModelRateLimitRoute.Use(middleware.AdminAuth())
		{
			groupModelRateLimitRoute.GET("/", controller.GetGroupModelRateLimits)

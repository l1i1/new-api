...
func taskPluginProtocolHandlers(protocol, operation string) ([]gin.HandlerFunc, error) {
	...
	case "openai_video.content":
		// Tokeness: the dashboard plays task videos with a session, so the
		// content route accepts either auth (fork behavior); the controller
		// scopes access to owner/admin via getTaskForArtifactRequest.

/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { useState } from 'react'
import { useTranslation } from 'react-i18next'

import { SectionPageLayout } from '@/components/layout'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'

import { InvoiceApplyPanel } from './components/invoice-apply-panel'
import { InvoiceRecordsPanel } from './components/invoice-records-panel'

type InvoiceTab = 'apply' | 'records'

export function InvoicePage() {
  const { t } = useTranslation()
  const [activeTab, setActiveTab] = useState<InvoiceTab>('apply')
  const [recordsRefreshKey, setRecordsRefreshKey] = useState(0)

  const handleSubmitted = () => {
    setRecordsRefreshKey((key) => key + 1)
    setActiveTab('records')
  }

  return (
    <SectionPageLayout>
      <SectionPageLayout.Title>{t('Invoice')}</SectionPageLayout.Title>
      <SectionPageLayout.Content>
        <div className='mx-auto flex w-full max-w-7xl flex-col gap-4 sm:gap-5'>
          <Tabs
            value={activeTab}
            onValueChange={(value) => setActiveTab(value as InvoiceTab)}
          >
            <TabsList>
              <TabsTrigger value='apply'>
                {t('Apply for Invoice')}
              </TabsTrigger>
              <TabsTrigger value='records'>
                {t('Invoice Records')}
              </TabsTrigger>
            </TabsList>
            <TabsContent value='apply' className='pt-4'>
              <InvoiceApplyPanel onSubmitted={handleSubmitted} />
            </TabsContent>
            <TabsContent value='records' className='pt-4'>
              <InvoiceRecordsPanel refreshKey={recordsRefreshKey} />
            </TabsContent>
          </Tabs>
        </div>
      </SectionPageLayout.Content>
    </SectionPageLayout>
  )
}

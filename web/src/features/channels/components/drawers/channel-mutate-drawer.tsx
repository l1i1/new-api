...
export function ChannelMutateDrawer({
  ...
  const currentMultiKeyTestModel = formValues.multi_key_test_model
  const multiKeyTestEnabled = formValues.multi_key_test_enabled
  ...
  const multiKeyTestModelOptions = useMemo(() => {
    const models = currentMultiKeyTestModel?.trim()
      ? [...new Set([...currentModelsArray, currentMultiKeyTestModel])]
      : currentModelsArray
    return models.map((model) => ({
      value: model,
      label: model,
    }))
  }, [currentModelsArray, currentMultiKeyTestModel])
  ...
  const ollamaCacheEstimationField = currentType === 4 && (
    <FormField
      control={form.control}
      name='ollama_cache_estimation_enabled'
      render={({ field }) => (
        <FormItem className='flex items-center justify-between px-4 py-3'>
          <div className='space-y-0.5'>
            <FormLabel>{t('Ollama Prompt Cache Estimation')}</FormLabel>
            <FormDescription>
              {t(
                'Estimate cached prompt tokens via message-prefix matching when upstream omits cached_tokens'
              )}
            </FormDescription>
          </div>
          <FormControl>
  ...
  const officialFitModelsField = (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>{t('Official-fit model allowlist')}</FormLabel>
          <FormControl>
            <Input
              placeholder={t('e.g., deepseek-v4.1-flash, deepseek-v4-flash')}
              {...field}
            />
          </FormControl>
          <FormDescription>
  ...
  const multiKeyScheduledTestFields =
    (isMultiKeyChannel || isConvertingToMultiKey) && (
      <div className='border-border/60 rounded-lg border'>
        ...
        <div className='divide-border space-y-0 divide-y border-t'>
          <FormField
            control={form.control}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='px-4 py-3'>
                    ...
                    <FormDescription>
                      {t('How frequently the system probes keys of this channel')}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='px-4 py-3'>
                    ...
                    <Select
                      disabled={multiKeyTestModelOptions.length === 0}
                      items={multiKeyTestModelOptions}
                    ...
                    >
                      ...
                      <SelectContent alignItemWithTrigger={false}>
                        <SelectGroup>
                          {multiKeyTestModelOptions.map((option) => (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='flex items-center justify-between px-4 py-3'>
                    <div className='space-y-0.5'>
                      <FormLabel>
                        {t('Re-enable manually disabled keys')}
                      </FormLabel>
                      <FormDescription>
                        {t(
                    ...
                    <FormItem className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                      ...
                      <Select
                        ...
                        items={addModeOptions
                          .filter(
                            (option) => !isEditing || option.value !== 'batch'
                          )
                          .map((option) => ({
                            value: option.value,

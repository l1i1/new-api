...
test('quick options and detailed settings share changes across tabs and save the same configuration', async () => {
  ...
  render(<ConfigurationHarness currentRow={editingChannel} />)
  await screen.findByDisplayValue('Existing channel')
  const dialog = within(screen.getByRole('dialog', { name: 'Edit Channel' }))
  const quick = within(dialog.getByRole('group', { name: 'Quick options' }))
  for (const name of [
    'Pass Through Body',
  ...
  fireEvent.click(quick.getByRole('switch', { name: 'Auto-disable channel' }))

  fireEvent.click(dialog.getByRole('tab', { name: /Request & Response/ }))
  expect(
    dialog.getByRole('switch', { name: 'Enable Responses WebSocket' })
  ).not.toBeChecked()
  const passthrough = dialog.getByRole('switch', { name: 'Pass Through Body' })
  ...
  await waitFor(() => expect(passthrough).toBeChecked())
  expect(dialog.getByRole('switch', { name: 'Force Format' })).toBeVisible()
  expect(
    dialog.getByRole('switch', { name: 'Allow service_tier passthrough' })
  ).toBeVisible()

  fireEvent.click(dialog.getByRole('tab', { name: /Other Settings/ }))
  const modelCheck = dialog.getByRole('switch', {
    name: 'Upstream Model Update Check',
  })
  expect(modelCheck).not.toBeChecked()
  expect(
    dialog.getByRole('switch', { name: 'Auto Sync Upstream Models' })
  ).toHaveAttribute('aria-disabled', 'true')
  fireEvent.click(modelCheck)
  expect(
    dialog.getByRole('switch', { name: 'Auto Sync Upstream Models' })
  ).toBeChecked()

  fireEvent.click(dialog.getByRole('tab', { name: /Routing & Mapping/ }))
  expect(dialog.getByRole('switch', { name: 'Auto Ban' })).toBeChecked()
  fireEvent.click(dialog.getByRole('switch', { name: 'Auto Ban' }))
  fireEvent.click(dialog.getByRole('tab', { name: /Connection & Models/ }))
  ...
  fireEvent.click(dialog.getByRole('button', { name: 'Update Channel' }))
  await waitFor(() => expect(put).toHaveBeenCalled())

...
import { localizeApiKeyGroupDescriptions } from './api-key-group-options'
...
export function AutoGroupOrderEditor(props: AutoGroupOrderEditorProps) {
  ...
  const localizedGlobalOptions = useMemo(
    () => localizeApiKeyGroupDescriptions(props.globalOptions, i18n.language),
    [i18n.language, props.globalOptions]
  )
  ...
  return (
    ...
    >
        ...
        <p className='text-muted-foreground text-xs' aria-live='polite'>
          {isInheriting
            ? t('Using the complete global Auto order ({{count}} groups)', {
                count: localizedGlobalOptions.length,
              })
      ...
      {isInheriting && localizedGlobalOptions.length === 0 && (
      ...
      {isInheriting && localizedGlobalOptions.length > 0 && (
        ...
        >
          {localizedGlobalOptions.map((option, index) => (

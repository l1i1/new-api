...
const existingAutoApiKey: ApiKey = {
  id: 42,
  name: 'existing-auto-key',
  key: 'sk-test',
  status: 1,
  remain_quota: 0,
  used_quota: 0,
  ...
  accessed_time: 0,
  group: 'auto',
  auto_groups: ['g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', 'g8'],
...
async function renderUpdateDrawer(options: {
  apiKeyResponse?: Record<string, unknown>
  autoGroupsResponse?: Record<string, unknown>
}): Promise<void> {
  ...
  queryClient.setQueryData(
    ['token-auto-groups'],
    options.autoGroupsResponse ?? {
      success: true,
      data: { groups: ['g1', 'g2'], max_count: 10 },
    },
    { updatedAt: freshAt }
  )
  queryClient.setQueryData(
    ['api-key', existingAutoApiKey.id],
    options.apiKeyResponse ?? { success: true, data: existingAutoApiKey },
    { updatedAt: freshAt }
  )
...
describe('API keys mutate drawer Auto group integration', () => {
  ...
  test('blocks saving and offers retry when the Auto configuration fails to load', async () => {
    await renderUpdateDrawer({
      autoGroupsResponse: {
        success: false,
        message: 'Auto configuration unavailable',
      },
    })

    const saveButton = findButton('Save changes', true)
    assert.equal(saveButton.disabled, true)
    assert.ok(findButton('Retry', false))
    assert.equal(document.body.textContent?.includes('Failed to load'), true)
  })

  test('blocks saving and offers retry when the API key detail fails to load', async () => {
    await renderUpdateDrawer({
      apiKeyResponse: {
        success: false,

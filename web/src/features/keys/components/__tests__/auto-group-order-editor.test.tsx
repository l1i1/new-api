...
describe('Auto group order editor', () => {
  ...
  test('localizes inherited descriptions without exposing tnt markup', async () => {
    ...
    const localizedOptions = [
      {
        value: 'vip',
        label: 'VIP',
        desc: '<tnt l="zh">优先访问</tnt><tnt l="en">Priority access</tnt>',
        ratio: 3,
      },
    ]
    ...
    for (const [language, expected] of [
      ...
      await act(async () => {
        ...
        root.render(<InheritanceHarness globalOptions={localizedOptions} />)
      })
      ...
      assert.equal(chip.textContent?.includes('<tnt'), false)
    }

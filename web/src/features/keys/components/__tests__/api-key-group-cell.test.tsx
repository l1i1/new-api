...
describe('API key group table cell', () => {
  ...
  test.each([
    [0.8, '20% off', 'bg-info/10', 'text-info', 'border-info/30'],
    [3, 'x3', 'bg-warning/10', 'text-warning', 'border-warning/30'],
  ])(
    'preserves the original %s multiplier color in the compact layout',
    (ratio, label, background, color, border) => {
      const { container } = render(
        <CellHarness group='default' ratio={ratio} />
      )
      const multiplier = screen.getByText(label).closest('[data-slot="badge"]')
  ...
  test('keeps a long group name and exact multiplier available through keyboard focus', async () => {
    ...
    expect(screen.getByText('x12.35')).toBeInTheDocument()
    await userEvent.tab()
    expect(

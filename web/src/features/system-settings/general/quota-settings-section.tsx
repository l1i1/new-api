...
const MAX_INPUT_PRE_CONSUME_MULTIPLIER = 100
...
const quotaSchema = z.object({
  ...
  quota_setting: z.object({
    ...
    pre_consume_multiplier: z.coerce
      ...
      .max(MAX_INPUT_PRE_CONSUME_MULTIPLIER, {
        error: () =>
          i18next.t('Must be at most {{max}}', {
            max: MAX_INPUT_PRE_CONSUME_MULTIPLIER,
          }),
      }),
  }),
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    {t(
                      'Reserve a conservative input and output estimate for token prices, applying this value to the input component. Defaults to 1; positive decimals up to {{max}} are supported. Final charges use actual usage. Per-request and task prices are unaffected.',
                      { max: MAX_INPUT_PRE_CONSUME_MULTIPLIER }
                    )}
                  </FormDescription>
                  <FormMessage />

...
it.each([
  ['retry_keyword_matched', 'Retry keyword matched'],
  ['response_committed', 'Response already committed'],
])('renders the %s policy decision label', (reason, label) => {
  render(
      ...
      events={[
        {
          attempt: 0,
          elapsed_ms: 1,
          decision: { action: 'stop', reason, source: 'routing' },
        },
      ]}
    />
  )
  expect(
    screen.getByRole('list', { name: 'Request decision flow' })

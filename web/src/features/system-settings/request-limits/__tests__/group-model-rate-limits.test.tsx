...
const { GroupModelRateLimitsSection } =
  await import('../group-model-rate-limits-section')
...
describe('group model rate limits', () => {
  afterEach(() => {
  ...
  test('loads rules and submits the complete edited rule list', async () => {
    api.get = (async (url) => {
      if (String(url).startsWith('/api/group-model-rate-limits')) {
        return {
          data: {
            success: true,
            data: [
    ...
    await queryClient.prefetchQuery({
      ...
      queryFn: () =>
        api.get('/api/group-model-rate-limits').then((res) => res.data),
    })
    ...
    assert.equal(
      document.querySelector<HTMLInputElement>(
        '#group-model-rate-limit-group-0'
      )?.value,
      'default'
    )
    assert.equal(
      document.querySelector<HTMLInputElement>(
        '#group-model-rate-limit-model-0'
      )?.value,
      'gpt-test'
    )
    await act(async () => findButton('Add rule').click())
    const secondGroup = document.querySelector<HTMLInputElement>(
      '#group-model-rate-limit-group-1'
    )
    ...
    await act(async () => {
      ...
      valueSetter.call(secondGroup, 'vip')
      secondGroup.dispatchEvent(new Event('input', { bubbles: true }))
    })
    const secondModel = document.querySelector<HTMLInputElement>(
      '#group-model-rate-limit-model-1'
    )
    ...
    await act(async () => findButton('Save group model limits').click())

...
export function ModelPriceCell(props: {
  ...
  const usdExchangeRate =
    options.usdExchangeRate ?? currency.usdExchangeRate ?? 1
  let currencyLabel: string = options.displayCurrency ?? 'USD'
  if (!options.displayCurrency) {
    if (currency.quotaDisplayType === 'CNY') {
      currencyLabel = 'CNY'
    } else if (currency.quotaDisplayType === 'CUSTOM') {
      currencyLabel = currency.customCurrencySymbol || '¤'
    }
  }
  // The caption carries the currency, so the amount itself is rendered without
  // a symbol. Scaling follows the caption so the amount and its unit agree.
  const displayCurrency: PricingCurrency =
    options.displayCurrency ?? (currencyLabel === 'CNY' ? 'CNY' : 'USD')
  ...
  let metrics: Array<{ label: string; value: string; original?: string }>

...
describe('pricing controls', () => {
  it('counts each model once per filter and updates counts when the catalog changes', () => {
    ...
    const tagsContainer = screen.getByRole('button', {
      name: 'Model Tags',
    }).parentElement
    const endpointContainer = screen.getByRole('button', {
      name: 'Endpoint Type',
    }).parentElement
    if (!tagsContainer || !endpointContainer) {
      throw new Error('Pricing filter sections are missing')
    }
    const tagsSection = within(tagsContainer)
    const endpointSection = within(endpointContainer)
    ...
    expect(
      tagsSection.getByRole('button', { name: /^Chat\s*2$/ })
    ).toBeVisible()
    expect(
      endpointSection.getByRole('button', { name: /^Chat\s*1$/ })
    ).toBeVisible()
    expect(
    ...
    expect(
      tagsSection.getByRole('button', { name: /^Chat\s*1$/ })
    ).toBeVisible()
    expect(
      endpointSection.getByRole('button', { name: /^Chat\s*0$/ })
    ).toBeVisible()
    expect(

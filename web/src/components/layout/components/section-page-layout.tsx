...
export function SectionPageLayout(props: SectionPageLayoutProps) {
  ...
  return (
    <PageFooterProvider container={footerContainer}>
      <Main>
        <div className='border-border shrink-0 border-b px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4'>
          {breadcrumb != null && (
            <div className='mb-2 sm:mb-3'>{breadcrumb}</div>
          )}
          <div className='flex flex-wrap items-center justify-between gap-x-3 gap-y-2 sm:gap-x-4'>
            <div className='min-w-0 flex-1'>
              <h2 className='truncate text-lg font-semibold tracking-[-0.01em] sm:text-xl'>
                {title}
              </h2>
            </div>
            {actions != null && (
        ...
        <div
          className={
            props.fixedContent
              ? 'min-h-0 flex-1 overflow-hidden px-4 pt-4 pb-4 sm:px-6 sm:pt-5 sm:pb-6'
              : 'min-h-0 flex-1 overflow-auto px-4 pt-4 pb-4 sm:px-6 sm:pt-5 sm:pb-6'
          }
        >
          {content}
        </div>

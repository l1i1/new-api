...
if run_rollout "$noop_case" \
  ...
  TOKENESS_TEST_FAIL_HOST=149.13.91.236; then
...
assert_commands "$stale_command_case/commands.log" '103.214.68.250|verify'

mutable_case="$test_root/mutable"
if run_rollout "$mutable_case" TOKENESS_TEST_MUTABLE_HOST=103.214.68.250; then
  fail "mutable rollback image unexpectedly passed preflight"
fi
assert_commands "$mutable_case/commands.log" '103.214.68.250|verify'
...
for host in 103.214.68.250 149.13.91.236 216.73.158.156 23.95.248.94; do
...
assert_commands "$lock_case/commands.log" \
  ...
  '103.214.68.250|verify' \
  "103.214.68.250|deploy $OLD_IMAGE"
[[ "$(<"$lock_case/state/103.214.68.250")" == "$OLD_IMAGE" ]] ||

...
| Fit Policy（Phase 3）第一步：`pkg/fitpolicy` 是 **fork 独有包**，把族级 pin 谓词组合外提为可热加载数据（规则 JSON → expr 程序 → 原子替换快照）。**当前处于 shadow 阶段：策略只观测不改路**，`BuiltinPolicy()` 复刻既有 `deepseekV4RequestNeedsOfficial` / `kimiK3RequestNeedsOfficial` / GLM 整族 pin 的判定，等价由测试逐例保证，分歧只计数+记日志。上游 hook 三处：`model/option.go`（import、`validateOptionValue` 提交前编译拒绝、`loadOptionsFromDatabase` 里安装快照——该函数同时覆盖启动/epoch/周期同步，**不要改用独立 reload hook**）、`middleware/distributor.go`（`observeFitPolicyShadow` 调用）。上游没有这个目录，普通 merge 不会删它，但按上游树整体覆盖式解决冲突会让它整体消失——所以现在就登记 | `pkg/fitpolicy/*.go`、`model/fitpolicy_option.go`、`middleware/fitpolicy_shadow.go`、`model/option.go`、`middleware/distributor.go`；设计见 `docs/fitpolicy-tech-spec.md` | `pkg/fitpolicy/fitpolicy_test.go`（`TestBuiltinPolicyCoversEveryRegisteredFamily`、`TestDecideGating`、`TestCompileRejectsUnknownFunctionAndNonBooleanRule`、`TestReloadKeepsLastKnownGood`、`TestPrimitivesMatchShippedSemantics`）、`middleware/fitpolicy_equivalence_test.go`（`TestFitPolicyBuiltinMatchesShippedPredicates`、`TestFitPolicyBuiltinPinsGlmWholeFamily`、`TestFitPolicyBuiltinPinsNothingWhenRouteDisabled`）、`middleware/fitpolicy_shadow_test.go`（`TestFitPolicyShadowDoesNotChangePin`、`TestFitPolicyShadowReportsDivergenceWithoutRerouting`）、`model/fitpolicy_option_test.go`（`TestValidateFitPolicyOption`、`TestRefreshFitPolicySnapshotFollowsOptionMap`） |

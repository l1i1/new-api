...
| Fit Policy（Phase 3）第一步：`pkg/fitpolicy` 是 **fork 独有包**，把族级 pin 谓词组合外提为可热加载数据（规则 JSON → expr 程序 → 原子替换快照）。当前只落了纯逻辑层与等价对照，**尚未接线到选路**：`BuiltinPolicy()` 复刻既有 `deepseekV4RequestNeedsOfficial` / `kimiK3RequestNeedsOfficial` / GLM 整族 pin 的判定，门禁保证「策略有意见 ⟺ 既有谓词 pin」逐例一致。上游没有这个目录，普通 merge 不会删它，但按上游树整体覆盖式解决冲突会让它整体消失——所以现在就登记 | `pkg/fitpolicy/*.go`；设计见 `docs/fitpolicy-tech-spec.md` | `pkg/fitpolicy/fitpolicy_test.go`（`TestBuiltinPolicyCoversEveryRegisteredFamily`、`TestDecideGating`、`TestCompileRejectsUnknownFunctionAndNonBooleanRule`、`TestReloadKeepsLastKnownGood`、`TestRegisterReloadHookRunsOnlyOnce`、`TestPrimitivesMatchShippedSemantics`）、`middleware/fitpolicy_equivalence_test.go`（`TestFitPolicyBuiltinMatchesShippedPredicates`、`TestFitPolicyBuiltinPinsGlmWholeFamily`、`TestFitPolicyBuiltinPinsNothingWhenRouteDisabled`） |

...
// recentSuccessSeries returns one timestamped point per hour that had requests,
// aligned to hour start regardless of the configured bucket size. The model
// card anchors its 24 slots to the client's current hour and places each point
// by timestamp, leaving hours without traffic gray.

...
3. For paid token-priced expressions, evaluates the frozen expression twice with the same estimated input: `F = expr(P, C_est, Len)` and `I = expr(P, 0, Len)`. `C_est` is the explicit `max_tokens` estimate, or `8192` when a paid request omits it. The raw reservation is `max(F, F + (m - 1) * I)`, where `m` is `quota_setting.pre_consume_multiplier` in the range `(0, 100]`, with 0.5, 1.5 and other positive decimals supported in the UI (default 1). This preserves the full-output estimate when `m < 1` and applies the multiplier only to the input component. If request token estimation is disabled, the estimated input is zero.
4. Converts the reservation to quota with `rawCost / 1,000,000 * QuotaPerUnit`. Request-priced leaves reserve `F` only and are never multiplied. Free groups reserve zero. The snapshot freezes `C_est` and `m` for image quantity retries; group retries reuse the resulting reservation. Older snapshots with no multiplier retain a factor of 1.
...
Wallet trust bypass uses `quota_setting.trust_quota_usd` (default 10 USD; 0 disables bypass). Both the wallet and any limited API token must exceed the threshold. The wallet must still cover the estimated reservation before bypass is considered. Subscriptions and asynchronous tasks do not use this bypass. Legacy token-ratio pricing retains the fork's conservative input policy: it reserves `max(promptTokens, PreConsumedQuota) * multiplier` for input and the configured `max_tokens * completionRatio` separately. Task usage pricing is unchanged.

...
Supported extraction forms are OpenAI Chat `image_url` objects or strings, OpenAI Responses `input_image.image_url`, Anthropic URL sources and Base64 sources with `media_type`, and Gemini HTTP(S) `fileData.fileUri` plus Base64 `inlineData`. Only HTTP(S) URLs and `data:image/*` URLs are forwarded. Duplicate images are removed before one image is randomly selected. Only the final conversation item is eligible: assistant/model turns and tool/function response items produce no moderation input instead of falling back to older user content. This follows the OpenAI multimodal moderation contract documented at <https://developers.openai.com/api/docs/guides/moderation> and the bounded one-image behavior used by Sub2API.

...
The complete normalized latest-user turn is preserved. Turns longer than 16,000 runes are split into an OpenAI-compatible `input` array and sent in one moderation request. The response must contain exactly one result per submitted input before the service aggregates the maximum score per category and the flagged state. Empty or partial result sets remain fail-open but retryable and cannot create a cached allow decision.

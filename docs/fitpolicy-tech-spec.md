...
| **A（代码已落，观测期未开始）** | 已实现：`pkg/fitpolicy`（schema/校验/expr 编译/原子快照/`Decide`→`Requirement`/两阶段 `Narrow`）、`constant.ContextKeyFitRequirement`、`model.FitChannelFilterForRequest`、`GetRandomSatisfiedChannelPinnedWithFit` 与 `GetChannelWithBlockedChannelsPinnedWithFit`（additive，旧签名保留）、`service` 两处改调 WithFit、`middleware.applyFitPolicy`（in-scope 精确路径 + 显式 pin 短路 + shadow 不写 context）、`middleware.fitPolicyAllowsAffinity`（affinity 直选分支的 fit eligibility）、`loadOptionsFromDatabase` 安装快照（缺口 1/2 均已接线，见 §9）。**注意与原文的两处偏离**：① 用「并入 `loadOptionsFromDatabase`」代替「`main.go` 显式 `RegisterReloadHook`」——前者同时覆盖启动/epoch/周期同步，后者只覆盖 epoch；② shadow 期间 requirement **不写入** gin context，使「策略不生效」成为结构性保证而非约定。**尚未开始**：≥48h 真实观测期、`getChannel` 复用分支回归测试、三库矩阵 | 见上；外加 A 阶段已落地证据：`pkg/fitpolicy` 与既有 Go 谓词 24 例逐例等价、`Narrow` 在无 mark 数据时等于官方硬 pin（`TestNarrowMatchesLegacyOfficialPin`）、`TestFitAwareSelectionNarrowsToMarkedChannels` 证明有 mark 时收窄真的生效、`go test ./...` 全绿 |
...
| **B（代码已落，三项待补）** | 已实现：`model/channel_fit_capability.go`（表/字段/唯一键/CAS/首插竞态/sticky+force/状态机/hash 绑定）、`model/channel_fit_capability_index.go`（索引 fail-open，随 `InitChannelCache` 重建）、`controller/channel_fit_capability.go`（专用 endpoint，`expected_revision` 必填，409 显式返回）、`service/authz/resources_channel.go`（`capability.write` 默认无角色、`capability.force` 默认 admin）、路由刻意注册在 `AdminAuth` 组**之外**、渠道删除/批量删除同事务清理、审计 best-effort。**鉴权选项选定为「dashboard 用户/PAT + 专用 action」**（规范给出的三种之一）：suite applier 持 `capability.write` 即可，不需要 `AdminAuth`。**审查修复（2026-09-26）**：① 能力写入必须指向存在的渠道（此前可造孤儿行）；② 索引状态改为**查询时**用当前时钟 + 请求自身的 policy/baseline 重算（`FitMarkRequirement`）——此前在构建时以空绑定冻结，使「规则/基线变更让旧报告 stale」这条规范要求实际失效，且过期判定最长滞后一个同步周期；③ suite 行自带 `expires_at` 未生效，现为 `suite_stale`；④ 报告未来 `generated_at` 会被夹到 now（否则造出永不过期标记）；⑤ GET 路由补回 `AdminAuth`；⑥ 策略新增 `baseline` 字段，使基线绑定真实生效（此前 `baseline_hash` 校验空转）。另修复一个审查中发现的上游潜在崩溃：`InitChannelCache` 在渠道有 group 但无 ability 时向 nil map 赋值 panic。**已补**：suite 报告校验与写回 applier（`pkg/fitpolicy/report.go` + `model/channel_fit_capability_report.go` + `POST /api/fit-capability/report`）：形状上限、族必须注册、行为名标识符形状、报告内去重、强制绑定 policy_version/policy_hash、逐项 CAS、单项冲突不丢整批、永不 force；`UpdateOption(s)` 现在在写节点立即安装快照。**尚未完成**：① MySQL/PostgreSQL 实跑（本机 docker socket 无权限且会话禁用审批，测试已写成 `TestFitCapabilityDatabaseMatrix`，缺 DSN 时 skip，需在有库的环境执行）；② 报告的**签名**验证（当前只校验绑定与形状，未校签；suite 侧签名与密钥分发未定）；③ 6h 校准任务本身（需要调度与部署）；④ 非 master 节点不建表的行为验证（依赖真实多节点部署，仅依赖既有 `if !common.IsMasterNode { return nil }` guard，未改动、未实测）。另：新增 resource action 使上游 `service/authz/authz_test.go` 的全量权限映射期望失效，已同步更新（机械性、语义中性） | SQLite 已实跑通过（含 fresh + 二次 AutoMigrate 幂等、唯一键、CAS 冲突、索引、删除清理）；MySQL/PG 待有库环境执行同一测试 |

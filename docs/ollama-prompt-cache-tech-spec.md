...
- Keep strict message-prefix matching, five-minute TTL, and upstream `cached_tokens` precedence.

## Acceptance

- A request without a client session field stores an Ollama prompt-cache entry.
- A follow-up request with the same token fallback and a strict message prefix receives estimated `cached_tokens` and estimated billing usage.

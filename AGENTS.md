...
- Edits billing expressions, quota math, pricing, request billing, task billing, usage derivation, settlement/refund code, or billing fields in relay info.
- Reads or writes `PriceData` / `OtherRatios`, quota pre-consume, settlement, refund, or consume-log billing fields anywhere else.
- Derives a billable quantity or `Usage` from an upstream response or stream, validates or forwards a billing multiplier, or adds/changes model prices or numeric task-plugin usage schema fields.
...
**Billing expression system:** When working on tiered/dynamic billing, follow `.agents/rules/billing.md` and read `pkg/billingexpr/expr.md` as required there. It documents the design philosophy, expression language, full architecture, token normalization rules, quota conversion, and expression versioning.
...
**These checks are executable, and they are a gate — not a ritual.** `node scripts/fork-invariants/main.mjs` implements the three checks below plus the inventory walk and the i18n invariants; `scripts/fork-invariants/README.md` explains each failure and how to fix it. CI runs it on every PR to `tokeness/main` and on every `sync/**` branch (`.github/workflows/tokeness-fork-invariants.yml`); a sync is not finished, and a release must not be cut, until it is green. Do not weaken an anchor, the orphan baseline, or the allowlist to make it pass — each of those three exists to record a decision, so record the decision and the reason.

**After every upstream sync, before treating the merge as done:**

1. **Compare the merge's parents against its result.** For merge `M` with fork parent `P1` and upstream parent `P2`, list every path where the result matches upstream while the fork side differed, and the reverse. Treat each hit as a suspected dropped fork change until verified by hand. A per-file comparison over `git ls-tree -r -z` is enough; reading every diff is not required. (`--check merge`, blocking.)
2. **Scan fork-only commits for surviving content.** For each commit reachable only from the fork side (`git log upstream..fork`), check whether the lines it added still exist in the merged tree. A low survival rate flags a dropped change. (`--check history`, advisory; `--fail-on-history-drop` to block.)
3. **Hunt orphaned producers.** A helper, comment, i18n key, or module with no remaining consumer almost always means a merge dropped its consumer. Grep for exported symbols with zero call sites and locale keys with zero `t()` usages. (`--check orphans`, blocking on *new* orphans against `orphan-baseline.json`.)

**Merge two-sided conflicts as a union, not a choice.** When both sides changed the same file, keep both behaviours unless one is provably superseded. `web/src/features/usage-logs/lib/format.ts` (fork's `getEffectiveBillingRatio` plus upstream's quota-operation delegation) is the known case. Locale files are no longer a two-sided conflict: they are upstream-owned, so the sync takes upstream's bytes verbatim and fork strings live in the overlay (below).
...
**Locale files after any merge:** `web/src/i18n/locales/*.json` are upstream-owned and must stay byte-identical to upstream — never add a fork string there. Fork strings and re-worded upstream strings go in `web/src/i18n/overlay/<locale>.json` (`translation` for keys upstream does not ship, `overrides` for keys we word differently), which `web/src/i18n/fork-bundles.ts` merges over the upstream bundle; tests and components import from there, not from `locales/`. The gate enforces: identical bundle hashes, identical fork key set across all seven locales, zero duplicate keys, no key defined in both maps, no fork key upstream has since adopted, no override whose key upstream removed. On a sync that legitimately takes upstream's bundles, re-record with `--record-locales`.

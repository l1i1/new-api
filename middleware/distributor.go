...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		channelIDValue, hasSpecificChannel := common.GetContextKey(c, constant.ContextKeyTokenSpecificChannelId)
		if pin, found, overridden := constraints.ResolvedPin(); found {
			...
			channelIDString, ok := channelIDValue.(string)
			if !ok {
				abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorInvalidChannelId))
				return
			}
			id, parseErr := strconv.Atoi(channelIDString)
			if parseErr != nil {
				abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorInvalidChannelId))
				return
			}

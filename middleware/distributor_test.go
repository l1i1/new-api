	...
	for _, locale := range []struct{ language, message string }{
		...
		for _, providerCount := range []int{1, 2} {
			t.Run(fmt.Sprintf("%s/%d_providers", locale.language, providerCount), func(t *testing.T) {
				...
				assert.JSONEq(t, fmt.Sprintf(`{"error":{"message":%q,"type":"server_error","param":null,"code":"server_error"}}`,
...
func TestDistributeSkipsHigherPriorityTaskPluginIdentityMismatch(t *testing.T) {
	require.NoError(t, i18n.Init())
	setupOriginTaskDB(t)
	...
	const modelName = "claimed-model"
	const pluginKey = "expected-plugin"
	highPriority := int64(10)
	lowPriority := int64(0)
	weight := uint(100)
	ordinary := &model.Channel{
		Name: "ordinary-high-priority", Key: "sk-ordinary", Status: common.ChannelStatusEnabled,
		Type: constant.ChannelTypeOpenAI, Group: "default", Models: modelName,
		Priority: &highPriority, Weight: &weight,
	}
	taskPlugin := &model.Channel{
		Name: "task-plugin-low-priority", Key: "sk-plugin", Status: common.ChannelStatusEnabled,
		Type: constant.ChannelTypeTaskPlugin, Group: "default", Models: modelName,
		Priority: &lowPriority, Weight: &weight,
	}
	taskPlugin.SetSetting(dto.ChannelSettings{TaskPluginKey: pluginKey})
	for _, channel := range []*model.Channel{ordinary, taskPlugin} {
	...
	assert.Equal(t, taskPlugin.Id, common.GetContextKeyInt(c, constant.ContextKeyChannelId))
}

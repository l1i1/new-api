...
func setupOriginTaskDB(t *testing.T) {
	...
	// Rate-limit tests in this package enable Redis without a client; the
	// group access policy load must take its database path here.
	common.RedisEnabled = false
	database, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	// The Tokeness distributor validates pinned channels against the group
	// access policy, which reads the abilities and policy tables.
	require.NoError(t, database.AutoMigrate(&model.Task{}, &model.Channel{}, &model.Ability{}, &model.GroupAccessPolicy{}))
	model.DB = database
...
func TestDistributeHonorsOriginTaskChannelPin(t *testing.T) {
	...
	// The Tokeness distributor validates the pinned channel against the
	// caller's group, mirroring the authenticated production path.

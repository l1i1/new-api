...
func UsageFromGeminiMetadata(metadata *dto.GeminiUsageMetadata, fallbackPromptTokens int) *dto.Usage {
	...
	if usage.BillingUsage == nil {
		...
		if usedFallbackPrompt {
			billingMetadata.PromptTokenCount = fallbackPromptTokens
		}
		if usedFallbackPrompt {
			if total := billingMetadata.PromptTokenCount + billingMetadata.ToolUsePromptTokenCount + billingMetadata.CandidatesTokenCount + billingMetadata.ThoughtsTokenCount; total > billingMetadata.TotalTokenCount {
				billingMetadata.TotalTokenCount = total
			}
		}
		usage.BillingUsage = dto.NewGeminiChatBillingUsage(&billingMetadata)
	}
	...
	if usedFallbackPrompt && usage.BillingUsage != nil && usage.BillingUsage.GeminiUsageMetadata != nil {
		usage = dto.MergeUsage(&dto.Usage{}, usage)
	}
	return usage
}

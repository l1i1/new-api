...
// Legacy password-update sentinels remain for older controller tests and
// callers while the profile endpoint uses model.ChangeUserPassword.
var (
	errUserPasswordUnset    = errors.New("user password is not set")
	errOriginalPasswordFail = errors.New("original password is incorrect")
)

func checkUpdatePassword(originalPassword, newPassword string, userID int) (bool, error) {
	if newPassword == "" {
		return false, nil
	}
	...
	if !common.ValidatePasswordAndHash(originalPassword, currentUser.Password) {
		return false, errOriginalPasswordFail
	}
	return true, nil
}
...
func Register(c *gin.Context) {
	...
	inviterId := model.ResolveInviterByAffCode(affCode)

...
func GetOptions(c *gin.Context) {
	...
	for k, v := range common.OptionMap {
		if k == "theme.frontend" || k == service.ContentModerationOptionKey ||
...
func UpdateOption(c *gin.Context) {
	...
	switch option.Key {
	case service.ContentModerationOptionKey:
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": "content moderation settings must use the dedicated endpoint",
		})
		return
	...
	case "AutomaticRetryStatusCodes", "ForceRetryStatusCodes", "NeverRetryStatusCodes", "MultiKeyCredentialRetryStatusCodes":
		_, err = operation_setting.ParseHTTPStatusCodeRanges(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,

...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t signature_present=%t payload_sha256=%s payload_bytes=%d", setting.CreemTestMode, signature != "", paymentPayloadDigestString(payload), len(payload)))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode signature_present=%t payload_sha256=%s payload_bytes=%d", signature != "", paymentPayloadDigestString(payload), len(payload)))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d payload_sha256=%s payload_bytes=%d", c.GetInt("id"), paymentPayloadDigest(bodyBytes), len(bodyBytes)))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s signature_present=%t payload_sha256=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), signature != "", paymentPayloadDigest(bodyBytes), len(bodyBytes)))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s payload_sha256=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), paymentPayloadDigest(bodyBytes), len(bodyBytes)))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s payload_sha256=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), paymentPayloadDigest(bodyBytes), len(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s error=%q payload_sha256=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), err.Error(), paymentPayloadDigest(bodyBytes), len(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}

...
func TestAddTokenRejectsInvalidAutoGroups(t *testing.T) {
	tests := []struct {
		...
		{name: "not globally enabled", maxCount: "5", autoGroups: `["default"]`, groups: []string{"vip"}},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			autoGroups := test.autoGroups
			if autoGroups == "" {
				autoGroups = `["default","vip"]`
			}
			configureTokenAutoGroupsTest(t, test.maxCount, autoGroups)
			user := setupTokenAutoGroupsControllerTest(t)

...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		decision := decideHTTPRelayRetry(c, newAPIError, common.RetryTimes-retryParam.GetRetry())
...
func getChannel(c *gin.Context, info *relaycommon.RelayInfo, retryParam *service.RetryParam) (*model.Channel, *types.NewAPIError) {
	...
	if limitErr := middleware.CheckGroupModelRateLimit(c, selectGroup, info.OriginModelName); limitErr != nil {
...
func decideHTTPRelayRetry(c *gin.Context, openaiErr *types.NewAPIError, retryTimes int) service.PolicyDecision {
	if openaiErr == nil {
		return service.DecideRelayRetry(c, nil, retryTimes)
	}
	...
	case helper.ResponsePayloadWritten:
		return service.PolicyDecision{Action: "stop", Reason: "response_committed", Source: "system"}
...
func shouldRetry(c *gin.Context, openaiErr *types.NewAPIError, retryTimes int) bool {
	return decideHTTPRelayRetry(c, openaiErr, retryTimes).Action == "retry"
}
...
func recordCyberPolicyErrorLog(c *gin.Context, channelError types.ChannelError, mark *service.CyberPolicyMark) {
	...
	other := model.NewLogOther()
	other.SetPublic("request_path", c.Request.URL.Path)
	other.SetPublic("error_type", "cyber_policy")
	other.SetPublic("error_code", "cyber_policy")
	other.SetPublic("status_code", mark.UpstreamStatus)
	other.SetPublic("upstream_status_code", mark.UpstreamStatus)
	other.SetPublic("error_source", "upstream_http")
	other.SetPublic("priority", "P3")
	other.SetPublic("is_business_limited", true)
	other.SetPublic("request_type", "cyber")
...
func taskSubmissionAPIError(taskErr *taskdto.TaskError) *types.NewAPIError {
	...
	if taskErr.LocalError {
		return types.NewErrorWithStatusCode(err, types.ErrorCodeBadResponseStatusCode, taskErr.StatusCode)
	}
...
func decideTaskRetry(c *gin.Context, taskErr *taskdto.TaskError, retryTimes int) service.PolicyDecision {
	...
	return service.DecideRelayRetry(c, taskSubmissionAPIError(taskErr), retryTimes)
}

// shouldRetryTaskRelay keeps the legacy test and extension entry point while
// routing every decision through decideTaskRetry.

...
func StripeWebhook(c *gin.Context) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 收到请求 path=%q client_ip=%s signature_present=%t payload_sha256=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), signature != "", paymentPayloadDigest(payload), len(payload)))

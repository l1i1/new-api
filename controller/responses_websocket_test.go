...
func TestResponsesStreamOutcomesPreserveAccounting(t *testing.T) {
	for _, transport := range []string{"websocket", "http-sse"} {
		for _, tc := range []struct {
			...
			{name: "failed-null-fixed", expression: `tier("request", fixed(0.002))`, terminal: `{"type":"response.failed","response":{"id":"first","status":"failed","usage":null,"error":{"code":"server_error","message":"sensitive upstream detail"}}}`, failed: true, httpRefund: true},
			{name: "failed-missing-fixed", expression: `tier("request", fixed(0.002))`, terminal: `{"type":"response.failed","response":{"id":"first","status":"failed"}}`, failed: true, httpRefund: true},
			...
			{name: "error-after-created", expression: `tier("request", fixed(0.002))`, terminal: `{"type":"error","status":500,"error":{"type":"server_error","code":"server_error","message":"Internal server error"}}`, sseTerminal: `{"type":"error","code":"server_error","message":"Internal server error","param":null,"sequence_number":2}`, failed: true, httpRefund: true},
			{name: "business-error-after-created", expression: `tier("request", fixed(0.002))`, terminal: `{"type":"error","status":400,"error":{"type":"invalid_request_error","code":"context_length_exceeded","message":"Input too long"}}`, sseTerminal: `{"type":"error","code":"context_length_exceeded","message":"Input too long","param":null,"sequence_number":2}`, failed: true, httpRefund: true, ignored: true},
			...
			t.Run(transport+"/"+tc.name, func(t *testing.T) {
				...
				// Preserve the fork HTTP refund contract for failed streams without
				// usage; accepted WebSocket requests retain their existing settlement.
				refunded := transport == "http-sse" && tc.httpRefund
				quotas := []int{1000}
				if refunded {
					quotas = nil
				}
				...
				// Health sampling counts request outcomes, independently of charges.
...
func TestResponsesHTTPHealthCountsFinalResult(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.firstStatus == http.StatusBadRequest {
				// The shared policy defaults to retrying upstream 400 responses: the
				// failover list covers 300-407, and the retired force-retry list was
				// folded into it. These health cases exercise the final 400 result,
				// so make the fixture's routing policy explicit instead of changing
				// the production default.
				previousFailover := operation_setting.AutomaticRetryStatusCodeRanges
				previousForceRetry := operation_setting.ForceRetryStatusCodeRanges
				operation_setting.AutomaticRetryStatusCodeRanges = nil
				operation_setting.ForceRetryStatusCodeRanges = nil
				t.Cleanup(func() {
					operation_setting.AutomaticRetryStatusCodeRanges = previousFailover
					operation_setting.ForceRetryStatusCodeRanges = previousForceRetry
				})
			}
			...
			if tc.success {
				// A failed upstream attempt is excluded before the retry. Give the
				// fixture a second real candidate so the retry tests the final
				// upstream result instead of a selection failure.
				modelMapping := `{"ws-billing":"gpt-4o"}`
				retryChannel := &model.Channel{
					Name: "responses-ws-retry", Key: "retry-key", Status: common.ChannelStatusEnabled,
					Type: constant.ChannelTypeOpenAI, Group: "default", Models: "ws-billing", BaseURL: &upstream.URL,
					ModelMapping: &modelMapping,
				}
				retryChannel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
				retryChannel.SetOtherSettings(dto.ChannelOtherSettings{AllowServiceTier: true})
				require.NoError(t, model.DB.Create(retryChannel).Error)
				require.NoError(t, model.DB.Create(&model.Ability{ChannelId: retryChannel.Id, Model: "ws-billing", Group: "default", Enabled: true}).Error)
				t.Cleanup(func() {
					require.NoError(t, model.DB.Where("channel_id = ?", retryChannel.Id).Delete(&model.Ability{}).Error)
					require.NoError(t, model.DB.Delete(retryChannel).Error)
				})
			}

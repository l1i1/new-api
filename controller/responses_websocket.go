...
var responsesWSRequestEngine = sync.OnceValue(func() *gin.Engine {
	...
	engine.POST("/v1/responses", func(c *gin.Context) {
		...
		prepareResponsesWSRequestContext(c)
		if c.IsAborted() {
			return
		}
		c.Next()
	}, middleware.UserModelRateLimit(), middleware.GroupModelRateLimit(), func(c *gin.Context) {
		state := c.Request.Context().Value(responsesWSRequestContextKey{}).(*responsesWSRequestState)
...
// prepareResponsesWSRequestContext supplies the model before the per-user and
// per-group limiters run. The normal HTTP distributor does this while it
// selects a channel; Responses WebSocket selection happens later, inside the
// persistent session, so the lightweight pre-pass must keep the limiters in
// the same request context without selecting a channel twice.
func prepareResponsesWSRequestContext(c *gin.Context) {
	...
	if modelName == "" && len(envelope.Response) > 0 {
		var nested struct {
			Model string `json:"model"`
		}
		if common.Unmarshal(envelope.Response, &nested) == nil {
	...
	if modelName != "" && userGroup != "" {
		if _, loaded := service.GetGroupAccessPolicy(c); !loaded {
			if err := service.LoadGroupAccessPolicy(c, userGroup); err != nil {
				logger.LogError(c, fmt.Sprintf("responses websocket group access policy lookup failed: %v", err))
				c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": gin.H{
					"message": common.MessageWithRequestId("database error", c.GetString(common.RequestIdKey)),
					"type":    "server_error",
				}})
			}
		}

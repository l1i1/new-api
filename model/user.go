...
func inviteUser(inviterId int) (err error) {
	...
	// Registration attribution is independent from payment compliance and the
	// legacy fixed inviter-quota setting. The latter may be zero while the
	// wallet still needs to show the number of users who registered through the
	// invite link. Use atomic expressions so concurrent registrations cannot
...
// DeleteUserForSession deletes the authenticated user's account after
// revalidating the session and auth-version fence in the same transaction.

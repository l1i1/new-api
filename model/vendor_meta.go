...
func (v *Vendor) Update() error {
	err := metadataTransaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Vendor{}).Where("id = ?", v.Id).Updates(map[string]any{"name": v.Name, "display_name": v.DisplayName, "description": v.Description, "icon": v.Icon, "updated_time": v.UpdatedTime}).Error
	})
	if err == nil {
		v.Version = VendorRecordVersion(v)
		RefreshPricing()

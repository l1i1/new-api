...
func requestPolicyDefaultOptions() map[string]string {
	...
	defaults["ForceRetryStatusCodes"] = operation_setting.ForceRetryStatusCodesToString()
	defaults["NeverRetryStatusCodes"] = operation_setting.NeverRetryStatusCodesToString()
	defaults["MultiKeyCredentialRetryStatusCodes"] = operation_setting.MultiKeyCredentialRetryStatusCodesToString()
	defaults["AutomaticRetryKeywords"] = operation_setting.AutomaticRetryKeywordsToString()
	defaults["NeverRetryKeywords"] = operation_setting.NeverRetryKeywordsToString()
	defaults["MultiKeyCredentialRetryKeywords"] = operation_setting.MultiKeyCredentialRetryKeywordsToString()
...
func CurrentRequestPolicy() *RequestPolicySnapshot {
	...
	if requestPolicySnapshot.CompareAndSwap(nil, snapshot) {
		return snapshot
	}
	return requestPolicySnapshot.Load()
}
...
func BuildRequestPolicy(options map[string]string) (*RequestPolicySnapshot, error) {
	...
	snapshot.NeverRetryKeywords = parseRequestPolicyKeywords(raw["NeverRetryKeywords"])
	for _, key := range []string{"ForceRetryStatusCodes", "NeverRetryStatusCodes", "MultiKeyCredentialRetryStatusCodes"} {
		if _, err := operation_setting.ParseHTTPStatusCodeRanges(raw[key]); err != nil {
...
func parseRequestPolicyKeywords(raw string) []string {
	keywords := make([]string, 0)
	for keyword := range strings.SplitSeq(raw, "\n") {
...
// MatchesNeverRetryKeywords checks the immutable, validated keyword snapshot
// used by relay decisions. Callers cannot change the configured list through
// the settings API after this snapshot has been published.
func (snapshot *RequestPolicySnapshot) MatchesNeverRetryKeywords(text string) bool {
	if snapshot == nil || text == "" || len(snapshot.NeverRetryKeywords) == 0 {
		return false
	}
	lower := strings.ToLower(text)
	for _, keyword := range snapshot.NeverRetryKeywords {

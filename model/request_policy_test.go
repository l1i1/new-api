	...
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			require.NoError(t, UpdateRequestPolicyOptions(map[string]string{
				"RetryTimes":                         "2",
				"channel_affinity_setting.rules":     rules,
				"ForceRetryStatusCodes":              "400,422",
				"NeverRetryStatusCodes":              "504,524",
				"MultiKeyCredentialRetryStatusCodes": "402,429",
				"AutomaticRetryKeywords":             "endpoint not supported\nmodel not found",
			...
			assert.Equal(t, "400,422", CurrentRequestPolicy().Options["ForceRetryStatusCodes"])
			assert.Equal(t, "504,524", CurrentRequestPolicy().Options["NeverRetryStatusCodes"])
			assert.Equal(t, "402,429", CurrentRequestPolicy().Options["MultiKeyCredentialRetryStatusCodes"])
			assert.Equal(t, "endpoint not supported\nmodel not found", CurrentRequestPolicy().Options["AutomaticRetryKeywords"])
			assert.Equal(t, []string{"context length", "prompt is too long"}, CurrentRequestPolicy().NeverRetryKeywords)
			...
			assert.Equal(t, "402,429", CurrentRequestPolicy().Options["MultiKeyCredentialRetryStatusCodes"])
			assert.Equal(t, "endpoint not supported\nmodel not found", CurrentRequestPolicy().Options["AutomaticRetryKeywords"])
			assert.Equal(t, "context length\nprompt is too long", CurrentRequestPolicy().Options["NeverRetryKeywords"])
			...
			for _, key := range []string{"ForceRetryStatusCodes", "NeverRetryStatusCodes", "MultiKeyCredentialRetryStatusCodes"} {
				assert.Error(t, UpdateRequestPolicyOptions(map[string]string{key: "599-500"}), key)
				assert.Same(t, snapshot, CurrentRequestPolicy())
			}

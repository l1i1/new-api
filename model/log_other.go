...
func formatLogOtherJSON(value string, visibility logOtherVisibility) string {
	...
	if visibility == logOtherVisibilityUser {
		...
		if rawStreamStatus, exists := values["stream_status"]; exists {
			var streamStatus map[string]json.RawMessage
			if err := common.Unmarshal(rawStreamStatus, &streamStatus); err == nil {
				for _, key := range []string{"end_error", "errors"} {
					if _, present := streamStatus[key]; present {
						delete(streamStatus, key)
						changed = true
					}
				}
				if encoded, err := common.Marshal(streamStatus); err == nil {

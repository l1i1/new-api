...
func (s *responsesWSSession) runRequest(state *responsesWSCallState, message []byte, envelope responsesWSCreateEvent, streamID string, requestID string) {
	...
	if parseErr != nil {
		apiErr = newResponsesWSInvalidRequestError(parseErr)
		return
	}
		...
		for ; retry.GetRetry() <= common.RetryTimes; retry.IncreaseRetry() {
			...
			if dialErr != nil {
				...
				if decision.Action == "retry" {
					prepareResponsesWSRetryWithMessage(retry, channel, apiErr.StatusCode, apiErr.Error(), types.IsSkipRetryError(apiErr))
					continue
				}
				return apiErr
			}
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	...
	if _, found, _ := constraints.ResolvedPin(); found && !service.GroupAccessPolicyAllowsSpecificChannel(c, channel, modelName, c.Request.URL.Path) {
...
func prepareResponsesWSRetry(retryParam *service.RetryParam, channel *appmodel.Channel, statusCode int, skipRetry bool) {
	prepareResponsesWSRetryWithMessage(retryParam, channel, statusCode, "", skipRetry)
}

func prepareResponsesWSRetryWithMessage(retryParam *service.RetryParam, channel *appmodel.Channel, statusCode int, message string, skipRetry bool) {

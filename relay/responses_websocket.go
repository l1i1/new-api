...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	if s.lockedChannelID != 0 {
		...
		if apiErr = checkResponsesWSContentModeration(c, info); apiErr != nil {
			return apiErr
		}
		...
		for ; retry.GetRetry() <= common.RetryTimes; retry.IncreaseRetry() {
			...
			if apiErr != nil {
				if service.IsMultiKeyRetryExhausted(apiErr) && info != nil && info.LastError != nil {
					retry.CancelRetryReset()
					retry.ClearPreferredChannel()
					retry.ExcludeChannel(common.GetContextKeyInt(c, appconstant.ContextKeyChannelId))
					apiErr = info.LastError
					continue
				}
				return apiErr
			}
			...
			slotRelease, saturated := service.AcquireChannelConcurrency(c, channel)
			if saturated {
				retry.ExcludeSaturatedChannel(channel.Id)
				retry.ResetRetryNextTry()
				apiErr = types.NewErrorWithStatusCode(
					fmt.Errorf("model %s channel concurrency limit reached", modelName),
			...
			for _, key := range []appconstant.ContextKey{
				...
				appconstant.ContextKeyChannelCredentialId, appconstant.ContextKeyChannelProxyMode, appconstant.ContextKeyChannelEffectiveProxy,
			} {
				if value, ok := c.Get(string(key)); ok {
					s.lockedContext[key] = value
				}
			}
			// Provider-specific routing values are stored under legacy string keys
			// rather than ContextKey constants. Preserve them for subsequent
			// response.create events on this locked connection.
			for _, key := range []string{"api_version", "region", "plugin", "bot_id", "platform", "task_plugin_key", "expected_task_plugin_key"} {
				if value, ok := c.Get(key); ok {
					s.lockedContext[appconstant.ContextKey(key)] = value
				}
			}
...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	...
	if channel.ChannelInfo.IsMultiKey {
		...
		if s.lockedKeyIndex >= 0 && s.lockedKeyIndex < len(keys) && keys[s.lockedKeyIndex] == s.lockedKey {
			status := common.ChannelStatusEnabled
			credential := channel.CredentialForPosition(s.lockedKeyIndex)
			if credential != nil {
				status = credential.Status
			} else if selectedStatus, ok := channel.ChannelInfo.MultiKeyStatusList[s.lockedKeyIndex]; ok {
				status = selectedStatus
			}
			keyEnabled = status == 0 || status == common.ChannelStatusEnabled
			if credentialID, ok := s.lockedContext[appconstant.ContextKeyChannelCredentialId].(int); ok && credentialID > 0 {
				keyEnabled = keyEnabled && credential != nil && credential.Id == credentialID
			}
		} else {
			keyEnabled = false
		}
	...
	if channel.ChannelInfo.IsMultiKey {
		if credential := channel.CredentialForPosition(s.lockedKeyIndex); credential != nil {
			effectiveProxy, proxyErr := credential.EffectiveProxyURL(currentSetting.Proxy)
			if proxyErr != nil {
			...
			currentSetting.Proxy = effectiveProxy
		}
	}
	if previous, ok := s.lockedContext[appconstant.ContextKeyChannelSetting].(dto.ChannelSettings); ok && previous.Proxy != currentSetting.Proxy {
		return types.NewErrorWithStatusCode(errors.New("upstream proxy changed; reconnect required"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
	if pin, found, _ := service.GetChannelConstraints(c).ResolvedPin(); found {
		...
		if !service.GroupAccessPolicyAllowsSpecificChannel(c, channel, model, c.Request.URL.Path) {
			return types.NewErrorWithStatusCode(errors.New("the connection channel is blocked by group access policy"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
		}
	} else {
		...
		if service.GroupAccessPolicyBlocksChannel(c, s.lockedChannelID) ||
			service.GroupAccessPolicyBlocksModel(c, model) ||
			!service.GroupAccessPolicyAllowsGroup(c, group) {
			return types.NewErrorWithStatusCode(errors.New("the connection channel is blocked by group access policy"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
		}
	...
	common.SetContextKey(c, appconstant.ContextKeyChannelSetting, currentSetting)
...
func (s *responsesWSSession) closeTarget() {
	s.connectionMu.Lock()
	target, unregister, slotRelease := s.target, s.unregister, s.slotRelease
	s.target, s.unregister, s.slotRelease = nil, nil, nil
...
func (s *responsesWSSession) setSlotRelease(release func()) {
	s.connectionMu.Lock()
	defer s.connectionMu.Unlock()
	if s.ctx.Err() != nil {
		if release != nil {
...
func checkResponsesWSContentModeration(c *gin.Context, info *relaycommon.RelayInfo) *types.NewAPIError {
	...
	content := service.ExtractContentModerationContent(info.Request, service.ContentModerationProtocolOpenAIResponses)
	request := service.ContentModerationRequest{
		UserID: info.UserId, Group: info.UsingGroup, Model: info.OriginModelName,
		Protocol:    service.ContentModerationProtocolOpenAIResponses,
		RequestPath: c.Request.URL.Path, RequestID: info.RequestId,
		Text: content.Text, Images: content.Images,
		ContentValidationError: content.ValidationError, ContentValidated: content.IsValidated(),
	}
	if policy, loaded := service.GetGroupAccessPolicy(c); loaded {
		request.GroupPolicyFingerprint = policy.Fingerprint
	}
	if affinity, ok := service.GetChannelAffinityStatsContext(c); ok {
		request.AffinityRuleName = affinity.RuleName
		request.AffinityKeyFingerprint = affinity.KeyFingerprint
		request.AffinityCacheIdentity = affinity.AffinityCacheIdentity
		request.AffinityTTLSeconds = int(affinity.TTLSeconds)
		request.AffinityChannelID = common.GetContextKeyInt(c, appconstant.ContextKeyChannelId)
	}
	decision, _ := service.SubmitContentModeration(c.Request.Context(), request)
	if decision == nil || !decision.Blocked || decision.Overloaded {
		return nil
	}
	...
	return types.NewErrorWithStatusCode(errors.New(message), types.ErrorCode("content_policy_violation"), statusCode, types.ErrOptionWithSkipRetry())
}
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	...
	if pin, found, overridden := constraints.ResolvedPin(); found {
		...
		if !service.GroupAccessPolicyAllowsSpecificChannel(c, channel, modelName, c.Request.URL.Path) {
			return nil, types.NewErrorWithStatusCode(errors.New("specified channel is blocked by group access policy"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
		}
	...
	if retryParam.GetRetry() == 0 {
		if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, modelName, usingGroup); found {
			...
			if err == nil && preferred != nil && preferred.Status == common.ChannelStatusEnabled &&
				!service.GroupAccessPolicyBlocksChannel(c, preferred.Id) &&
				!service.GroupAccessPolicyBlocksModel(c, modelName) {
				...
				if usingGroup == "auto" {
					...
					for _, g := range service.GetRequestAutoGroups(c, userGroup) {
						if appmodel.IsChannelEnabledForGroupModel(g, modelName, preferred.Id) &&
							service.GroupAccessPolicyAllowsGroup(c, g) {
							common.SetContextKey(c, appconstant.ContextKeyAutoGroup, g)
							service.MarkChannelAffinityUsed(c, g, preferred.Id)
							if apiErr := checkResponsesWSGroupRateLimit(c, modelName, preferred); apiErr != nil {
								return nil, apiErr
							}
					...
					service.GroupAccessPolicyAllowsGroup(c, usingGroup) {
	...
	if preferredChannelID := retryParam.PreferredChannelID(); preferredChannelID > 0 {
		channel, err := appmodel.CacheGetChannel(preferredChannelID)
		if err != nil || channel == nil || channel.Status != common.ChannelStatusEnabled ||
			...
			if selectGroup == "auto" {
				selectGroup = common.GetContextKeyString(c, appconstant.ContextKeyAutoGroup)
			}
			if selectGroup == "" || !service.GroupAccessPolicyAllowsGroup(c, selectGroup) ||
				!appmodel.IsChannelEnabledForGroupModel(selectGroup, modelName, channel.Id) {
				retryParam.ClearPreferredChannel()
			} else if ok, kind := appmodel.ChannelSatisfiesFilters(channel, modelName, constraints.Filters); !ok {
				return nil, types.NewErrorWithStatusCode(errors.New("preferred channel does not satisfy request constraints"), types.ErrorCode(kind), http.StatusBadRequest, types.ErrOptionWithSkipRetry())
			} else if err := middleware.SetupContextForSelectedChannel(c, channel, modelName); err != nil {
				return nil, err
			} else if apiErr := checkResponsesWSGroupRateLimit(c, modelName, channel); apiErr != nil {
				return nil, apiErr
			} else {
				return channel, nil
			}
...
func checkResponsesWSGroupRateLimit(c *gin.Context, modelName string, channel *appmodel.Channel) *types.NewAPIError {
	group := common.GetContextKeyString(c, appconstant.ContextKeyAutoGroup)
	if group == "" {
		group = common.GetContextKeyString(c, appconstant.ContextKeyUsingGroup)
	}
	if group == "auto" {
		...
		for _, candidate := range service.GetRequestAutoGroups(c, userGroup) {
			if service.GroupAccessPolicyAllowsGroup(c, candidate) &&
				appmodel.IsChannelEnabledForGroupModel(candidate, modelName, channel.Id) {
				group = candidate
				common.SetContextKey(c, appconstant.ContextKeyAutoGroup, group)
				break
			}
	...
	if limitErr := middleware.CheckGroupModelRateLimit(c, group, modelName); limitErr != nil {
		return types.NewErrorWithStatusCode(errors.New(limitErr.Message), types.ErrorCode(""), limitErr.StatusCode, types.ErrOptionWithSkipRetry())
	}
	return nil
}

func responsesWSConcurrencySaturatedError(modelName string) *types.NewAPIError {

...
func TestDeepSeekV4MessageContractValidationMatchesOfficial(t *testing.T) {
	...
	tests := []struct {
		...
		{"image on the text-only pro is accepted (official drift 2026-09-06: 200, not 400)", `{"model":"deepseek-v4-pro","messages":[{"role":"user","content":[{"type":"image_url","image_url":{"url":"https://example.com/x.png"}},{"type":"text","text":"这是什么？"}]}],"max_tokens":32}`, ""},

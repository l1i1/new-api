...
func TestSelectResponsesWSChannelAcceptsNativeResponsesChannelTypes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			channel := &model.Channel{Name: tc.name, Key: "sk-test", Status: common.ChannelStatusEnabled, Type: tc.channelType, Group: "default", Models: "ws-model"}
			...
			require.NoError(t, database.Create(channel).Error)
			require.NoError(t, database.Create(&model.Ability{ChannelId: channel.Id, Group: "default", Model: "ws-model", Enabled: true}).Error)
			c, _ := gin.CreateTestContext(httptest.NewRecorder())
			c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
			common.SetContextKey(c, constant.ContextKeyUserGroup, "default")
			common.SetContextKey(c, constant.ContextKeyUsingGroup, "default")
...
func TestRestoreConnectionContextRejectsChangedAdvancedCustomRoute(t *testing.T) {
	...
	channel := &model.Channel{Name: "advanced", Key: "sk-test", Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeAdvancedCustom, Group: "default", Models: "ws-model", BaseURL: &baseURL}
	channel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
